...
int FindWordsOfCount(int wordLength, string& strToSearch) {
	cout << "Initiating count for words of length: " << wordLength << endl;
	//cout << "." << endl;
...
int main() {
	...
	string easyTest = "This Sentence has words of varying lengths. They are over-emphasized numbers such as 4, 8, 3, 5, 2, 7, 7, 4, 3, 15, 7, 4, and 2s";
	bool continueRunning = true;
	int targetLength = 1;
	while (continueRunning) {
		cout << "Please Enter word length to search for (0 to quit): ";
		cin >> targetLength;
		if (targetLength == 0) {
		...
		if (pIDValue == 0) {
			...
			while (countCalled == 0) {
				countCalled = FindWordsOfCount(targetLength, easyTest);
			}
			cout << "Word length: " << targetLength << " \nAmount found: " << countCalled << endl;
		}
		else {
			cout << "Child pID: " << pIDValue << endl;
		}
